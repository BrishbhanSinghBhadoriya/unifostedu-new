...
const CoursesPage = async ({ params }: { params: { courses: string } }) => {
  ...
  const coursesData = (course as Record<string, courseType>)[courses];
  ...
  const { about, approval, whoCanApply, specializations, courseFees, certifications, syllabus_curriculum, placementPartners, faq, studentReview } = coursesData;
              ...
              {approval?.degreeRecognition && Array.isArray(approval.degreeRecognition) && (
                <div className="mt-4">
                  ...
                  <ul className="space-y-1 text-xs text-gray-700 list-disc list-inside">
                    {approval.degreeRecognition.map((item) => (
                      <li key={item}>{item}</li>
                    ))}
                  </ul>
                </div>
                ...
                <div>
                  ...
                  {whoCanApply.idealCandidates && Array.isArray(whoCanApply.idealCandidates) && (
                    <div className="mt-4">
                      ...
                      <ul className="space-y-1.5 text-sm text-gray-700 list-disc list-inside">
                        {whoCanApply.idealCandidates.map((item) => (
                          <li key={item}>{item}</li>
                        ))}
                      </ul>
                    </div>
                  ...
                  {whoCanApply.documentsRequired && Array.isArray(whoCanApply.documentsRequired) && (
                    <div className="mt-4">
                      ...
                      <ul className="space-y-1.5 text-sm text-gray-700 list-disc list-inside">
                        {whoCanApply.documentsRequired.map((doc) => (
                          <li key={doc}>{doc}</li>
                        ))}
                      </ul>
                    </div>
