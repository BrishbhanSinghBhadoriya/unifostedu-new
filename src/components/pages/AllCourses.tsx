...
import { FaGraduationCap, FaBriefcase, FaBook, FaLaptop, FaChartLine, FaNewspaper, FaUserGroup, FaClock, FaBuildingColumns, FaUserTie } from "react-icons/fa6";
import Link from "next/link";
import { Course } from "types/LandingPageTypes";

interface CourseProps {
  courses: Course[];
}

export function AllCourses({ courses = [] }: CourseProps) {

  const getCourseHref = (title: string) => {
  ...
  const getIcon = (course: Course) => {
    const iconKey = course.iconKey as string;
    ...
    if (title.includes("mca") || title.includes("bca") || title.includes("computer") || title.includes("tech")) return FaLaptop;
    ...
    if (title.includes("jmc") || title.includes("journalism")) return FaNewspaper;
                    ...
                    <div className="relative rounded-xl cursor-pointer overflow-hidden">
                      ...
                      <div className="relative z-10 bg-white rounded-xl border border-gray-200 p-4 sm:p-5 shadow-sm transition-all duration-300 group-hover:shadow-2xl group-hover:shadow-indigo-200 group-hover:translate-y-[-4px] group-hover:scale-[1.01] group-hover:bg-transparent group-hover:border-transparent">
                        ...
                        <div className="mt-2 pt-3 border-t border-gray-100 flex items-center justify-between text-[11px] sm:text-xs text-gray-600 group-hover:text-white/90 group-hover:border-white/20">
                          ...
                          <span className="flex items-center gap-1">
                            <FaUserGroup className="text-[#00ffe0]" />
                            {course.level}
                          </span>
                        </div>
                      </div>
