...
const SectionHeader = ({
  ...
  const renderTitle = () => {
    if (!highlight) return (
      <h2 className={`${sizeMap[size]} font-semibold tracking-tight text-[#001e3c] mb-2`}>{title}</h2>
    );
    const [before, after] = String(title).split(highlight);
    return (
  ...
  return (
    ...
    >
      ...
      {subtitle && (
        <p className={`text-base sm:text-lg text-gray-600 ${align === "center" ? "mx-auto" : ""} max-w-3xl px-4`}>{subtitle}</p>
      )}
    </motion.div>
  );
};
